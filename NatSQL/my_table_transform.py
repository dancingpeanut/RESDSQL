...
def table_transform(table, args, schema):
    ...
    for item in table['column_names']:
            ...
            if args.modify_column_names:
                ...
                if args.force_modify_star_column_names:
                    new_table['column_names'].insert(insert_index, [item[0],
                                                                    table['table_names'][item[0]].replace(" | ",
                                                                                                          " * | ") + ' *'])
                else:
        ...
        if args.modify_column_names and schema.column_tokens_lemma_str.count(
                schema.column_tokens_lemma_str[insert_index_schema]) > 1:
            for (tb_idx, last_table_string), last_table_lemma in zip(enumerate(table['table_names']),
                                                                     schema.table_tokens_lemma_str):
                last_table_string = last_table_string.split(" | ")
                last_table_lemma = last_table_lemma.split(" | ")
                if not new_table['column_names'][insert_index][1].startswith(last_table_string[0] + " ") and \
                        new_table['column_names'][insert_index][1] != last_table_string[0] \
                        and not schema.column_tokens_lemma_str[insert_index_schema].startswith(
                    last_table_lemma[0] + " ") and schema.column_tokens_lemma_str[insert_index_schema] != \
                        last_table_lemma[0] \
                        and (len(last_table_string) == 1 or (
                        not schema.column_tokens_lemma_str[insert_index_schema].startswith(
                            last_table_lemma[-1] + " ") and schema.column_tokens_lemma_str[insert_index_schema] !=
                        last_table_lemma[-1])) \
                        and " " + schema.column_tokens_lemma_str[insert_index_schema] not in last_table_lemma[0] and \
                        schema.column_tokens_lemma_str[insert_index_schema] + " " not in last_table_lemma[0]:
                    pass
                else:
                    for i2, col2 in enumerate(new_table['column_names']):
                        if new_table['column_names'][i2][1] == new_table['column_names'][insert_index][1] and \
                                new_table['column_names'][i2][0] == tb_idx:
            ...
            if last_table_string:
                ...
                if " | " not in new_table['column_names'][insert_index][1] and len(last_table_string) > 1:
                    new_table['column_names'][insert_index][1] = " | ".join(
                        [lts + " " + new_table['column_names'][insert_index][1] for lts in last_table_string])
                else:
        ...
        new_table['table_column_names_original'].append(
            [item[0], table['table_names_original'][item[0]] + "." + table['column_names_original'][item_index][1]])
        ...
        for jj in range(2):
            if index_table_in_table_list >= 0:
                ...
                next_tables, fk_next_tables = get_next_table(table_fk_left \
                                                             , table['index_range'], table['foreign_keys'],
...
def enlarge_network(net_work, table):
    def in_side_enlarge(t1t2_fk, table_1, table_2, table):
        ...
        for fk in table['foreign_keys']:
            ...
            fk.sort()
            if fk != t1t2_fk and (
                    (table['column_names'][fk[0]][0] == table_1 and table['column_names'][fk[1]][0] == table_2) or (
                    table['column_names'][fk[0]][0] == table_2 and table['column_names'][fk[1]][0] == table_1)):
                new_fk.append(fk)
        return new_fk
    ...
    def add_table_to_enlarge(net, idx, table_1, table_2, table, new_net_work):
        for i, t in enumerate(table['table_names_original']):
            if i not in net[1]:
                for fk in table['foreign_keys']:
                    if (table['column_names'][fk[0]][0] == table_1 and table['column_names'][fk[1]][0] == i) or (
                            table['column_names'][fk[0]][0] == i and table['column_names'][fk[1]][0] == table_1):
                        for fk2 in table['foreign_keys']:
                            if (table['column_names'][fk2[0]][0] == table_2 and table['column_names'][fk2[1]][
                                0] == i) or (table['column_names'][fk2[0]][0] == i and table['column_names'][fk2[1]][
                                0] == table_2):
    ...
    for i, net in enumerate(new_net):  # 3 tables network
        for j, net2 in enumerate(new_net):
            if i < j and ((net[1][1] in net2[1] and net[1][0] not in net2[1]) or (
                    net[1][0] in net2[1] and net[1][1] not in net2[1])):
                tmp_n3 = [[net[0][0], net2[0][0]],
                          [net[1][0], net[1][1], net2[1][0] if net2[1][1] in net[1] else net2[1][1]]]
    ...
    for i, net in enumerate(new_net):  # 4 tables network
        for j, net2 in enumerate(new_net_3):
            if (net2[1][-1] == net[1][0] and net[1][1] not in net2[1]) or (
                ...
                tmp_n4 = [[net2[0][0], net2[0][1], net[0][0]],
                          [net2[1][0], net2[1][1], net2[1][2], net[1][1] if net2[1][-1] == net[1][0] else net[1][0]]]
    ...
    for net in net_work:
        if len(net[1]) == 1 or len(net[1]) >= 5:
            ...
            for i, t in enumerate(table['table_names_original']):
                if i not in net[1]:
                    for fk in table['foreign_keys']:
                        if (table['column_names'][fk[0]][0] == table_idx and table['column_names'][fk[1]][0] not in net[
                            1]):
                            new_tb_idx = table['column_names'][fk[1]][0]
                        elif (table['column_names'][fk[0]][0] not in net[1] and table['column_names'][fk[1]][
                            0] == table_idx):
...
def check_col_table_similarity(_tokenizer, table_names, col_token):
    ...
    if table_token[-1].lemma_ == col_token[0].lemma_ or col_lemma == " ".join([tok.lemma_ for tok in table_token]) or \
            table_token[-1].lemma_ == col_token[0].text:
        return True
    return False
        ...
        for i, col in enumerate(all_cols):
            if col[1] == 0:  # boolean
                if tables[db_i]['column_types'][i + 1] != "boolean":
                    print(tables[db_i]['column_names'][i + 1][1] + " " + tables[db_i]['column_types'][
                        i + 1] + " --> boolean")
...
def remove_start_table(tables, schemas):
    for i, table, schema in zip(range(len(tables)), tables, schemas):
        ...
        for j, col in enumerate(table["column_names"]):
            if col[0] >= 0 and (col[1].startswith(schema.table_tokens_lemma_str[col[0]] + " ") or col[1].startswith(
                    schema.table_tokens_text_str[col[0]] + " ")):
                tables[i]["column_names"][j][1] = " ".join(
                    tables[i]["column_names"][j][1].split(" ")[schema.table_tokens_lemma_str[col[0]].count(" ") + 1:])
            elif j in table["primary_keys"] and tables[i]["column_names"][j][1].endswith(" id") and \
                    schema.table_tokens_lemma_str[col[0]].startswith(tables[i]["column_names"][j][1][0:-3]):
                tables[i]["column_names"][j][1] = tables[i]["column_names"][j][1][-2:]
            elif (tables[i]["column_names"][j][1] == schema.table_tokens_lemma_str[col[0]] or
                  tables[i]["column_names"][j][1] == schema.table_tokens_text_str[col[0]]) and \
                    tables[i]["column_types"][j] == "text" and "name" not in schema.table_col_lemma[col[0]]:
...
def recover_table_name(tables):
    for i, table in enumerate(tables):
        ...
        for j, col, ocol in zip(range(len(table["column_names"])), table["column_names"], table["old_column_names"]):
            if col[0] >= 0 and " | " not in col[1] and ocol[1].count(" ") > col[1].count(" ") and try_to_add.count(
                    col[0]) == 1:
...
def unifie_words(tables):
    ...
    for i, table in enumerate(tables):
        for j, col in enumerate(table["column_names"]):
            for w in WORDS:
                if w[0] == col[1] or col[1].startswith(w[0] + " ") or col[1].endswith(" " + w[0]) or " " + w[0] + " " in \
                        col[1]:
...
def analyse_same_column(tables, schemas, database_path):
    def they_are_same(col_1_idx, col_2_idx, table, schema, all_pair, db_):
        ...
        if [col_1_idx, col_2_idx] in all_pair or [col_2_idx, col_1_idx] in all_pair:
            return True
        if " " not in schema.column_tokens_lemma_str[col_1_idx] and " " not in schema.column_tokens_lemma_str[
            col_2_idx]:
            return False
        if col_1_idx != col_2_idx:
            if schema.column_tokens_lemma_str[col_1_idx] == schema.column_tokens_lemma_str[col_2_idx]:
                pass
            elif schema.table_tokens_lemma_str[table['column_names'][col_1_idx][0]] + schema.column_tokens_lemma_str[
                col_1_idx] == schema.column_tokens_lemma_str[col_2_idx]:
                pass
            elif schema.column_tokens_lemma_str[col_1_idx] == schema.table_tokens_lemma_str[
                table['column_names'][col_2_idx][0]] + schema.column_tokens_lemma_str[col_2_idx]:
                pass
            elif schema.table_tokens_lemma_str[table['column_names'][col_1_idx][0]] + schema.column_tokens_text_str[
                col_1_idx] == schema.column_tokens_text_str[col_2_idx]:
                pass
            elif schema.column_tokens_text_str[col_1_idx] == schema.table_tokens_text_str[
                table['column_names'][col_2_idx][0]] + schema.column_tokens_lemma_str[col_2_idx]:
                pass
            elif schema.table_tokens_text_str[table['column_names'][col_1_idx][0]] + schema.column_tokens_lemma_str[
                col_1_idx] == schema.column_tokens_lemma_str[col_2_idx]:
                pass
            elif schema.column_tokens_lemma_str[col_1_idx] == schema.table_tokens_text_str[
                table['column_names'][col_2_idx][0]] + schema.column_tokens_lemma_str[col_2_idx]:
                pass
            elif schema.column_tokens_text_str[col_1_idx] == schema.column_tokens_text_str[
                table['column_names'][col_2_idx][0]] + schema.column_tokens_text_str[col_2_idx]:
                pass
            elif schema.column_tokens_text_str[table['column_names'][col_1_idx][0]] + schema.column_tokens_text_str[
                col_1_idx] == schema.column_tokens_text_str[col_2_idx]:
                pass
            else:
                return False
            if schema.column_types[col_1_idx] == schema.column_types[col_2_idx] and contain_table_name(col_1_idx,
                                                                                                       col_2_idx, table,
                                                                                                       schema) and (
                    (db_ and db_.db_content_are_same(col_1_idx, col_2_idx)) or db_ is None):
...
def seperate_col_name(tables, all_words, schemas):
    suffix = {
        "ster", "sters", "ages", "bility", "cracy", "doms", "facture", "faction", "hood", "ices", "ions", "ours",
        "ship", "tude", "graphy", "graphs", "nomy", "hood", "ants", "ment", "some", "ward", "like", "fold", "most",
        "ways", "meter", "miter", "gram",  # "proof","able"
    }
    for i, table, schema in zip(range(len(tables)), tables, schemas):
        for j, col in enumerate(table["column_names"]):
            bk_bool = False
            if col[0] >= 0 and " " not in col[1] and len(col[1]) > 8:
                if col[1] not in all_words:
                    for x in range(4, len(col[1]) - 3):
                        ...
                        if col[1][0:x] in all_words and col[1][x:] in all_words and col[1][0:x] not in STOP_WORDS and \
                                col[1][x:] not in STOP_WORDS and col[1][x:] not in suffix:
                            print(tables[i]["column_names"][j][1] + " ==================> " +
                                  tables[i]["column_names"][j][1] + " | " + col[1][0:x] + " " + col[1][x:])
                            tables[i]["column_names"][j][1] += " | " + col[1][0:x] + " " + col[1][x:]
                            bk_bool = True
                            break
                if not bk_bool and schema.column_tokens_lemma_str[j] != col[1] and len(
                        schema.column_tokens_lemma_str[j]) > 8 and schema.column_tokens_lemma_str[j] not in all_words:
                    col_lemma = schema.column_tokens_lemma_str[j]
                    for x in range(4, len(schema.column_tokens_lemma_str[j]) - 3):
                        if col_lemma[0:x] in all_words and col_lemma[x:] in all_words and col_lemma[
                                                                                          0:x] not in STOP_WORDS and col_lemma[
                                                                                                                     x:] not in STOP_WORDS and col_lemma[
                                                                                                                                               x:] not in suffix:
                            print(tables[i]["column_names"][j][1] + " ==================> " +
        ...
        if db_infos:
            for db_info in db_infos:
                ...
                for line in lines:
                    ...
                    if line.strip().upper().endswith(" NOT NULL"):
                        line = line[:-8]
                    if line.strip().upper().endswith(" PRIMARY KEY,") or line.strip().upper().endswith(
                            " PRIMARY KEY ,") or line.strip().upper().endswith(" PRIMARY KEY"):
                        while True:
                            if line[0].isalpha():
                                break
                            else:
                        ...
                        ls = line.split(" ")
                        col = (db_info[1] + '.' + "".join(
                            [t if t.isalpha() or t == '_' else "" for t in ls[0]])).lower()
                        ...
                        if ',' in key_str:
                            ...
                            for k in key_str.split(','):
                                col = db_info[1].lower() + '.' + "".join(
                                    [t if t.isalpha() or t == '_' else "" for t in k])
...
def bridge_table_for_many2many_relationship(tables):
    for it, table in enumerate(tables):
        ...
        for net in table['network']:
            net2dict = dict()
            if len(net[1]) == 3 and len(net[0]) == 2 and net[0][0][0] not in net[0][1] and net[0][0][1] not in net[0][
                1]:
                for fks in net[0]:
...
def transfer_table_info(tables):
    args = construct_hyper_param()
    args.__setattr__('correct_col_type', False)
    args.__setattr__('remove_start_table', True)
    args.__setattr__('analyse_same_column', False)
    args.__setattr__('table_transform', True)
    args.__setattr__('correct_primary_keys', False)
    args.__setattr__('use_extra_col_types', True)
    print(args)

    database_path = './database1111'
...
if __name__ == '__main__':
    ...
    # 2. Prepare data
    tables = json.load(open('./data/CSpider/tables_1.json', 'r'))
    res = transfer_table_info(tables)
    print(res)
